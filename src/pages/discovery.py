...
growth_by_sector_scatter = html.Div([
    html.H4('Yearly Growth by Stock'),
    dcc.Graph(id="growth-scatter-plot")
])

growth_by_sector_icicle = html.Div([
    html.H4('Yearly Growth by Sector'),
    dcc.Graph(id="growth-icicle-chart")
])

analyst_recommendations_scatter = html.Div([
    html.H4('Analyst Recommendations'),
...
top_growth_stocks = html.Div([
                        ...
                        dash_table.DataTable(
                            ...
                            columns=[
                                ...
                                dict(id='close_pct_1yr', name='1yr Growth', type='numeric', format=percentage)
                            ],
        ...
        [
            dbc.Row(
                [
                    dbc.Col([
                        dbc.Row([
                            dbc.Col(html.Div(dcc.Dropdown(sectors, id='sector-dropdown', placeholder="Select a sector", multi=True), style=DROPDOWN_STYLE), width={"size": 2}),
                            dbc.Col(html.Div(dcc.Dropdown(industries, id='industry-dropdown', placeholder="Select an industry", multi=True), style=DROPDOWN_STYLE), width={"size": 2}),
                            dbc.Col(html.Div(dcc.Dropdown(tickers, id='ticker-dropdown', placeholder="Select a ticker", multi=True), style=DROPDOWN_STYLE), width={"size": 2})
                        ])
                    ], style=FILTER_STYLE, width={"size": 12})
...
def update_industry_dropdown(sector_options):
    if sector_options is None or sector_options == []:
        return industries
    new_industries = growth_df[growth_df['sector'].isin(sector_options)]['industry'].unique()
    new_industries.sort()
...
def update_ticker_dropdown(sector_options, industry_options):
    new_tickers = []
    if (sector_options == [] or sector_options == None) and (industry_options == [] or industry_options == None):
        return tickers
    elif len(sector_options)>=1 and (industry_options == [] or industry_options == None):
        new_tickers = growth_df[growth_df['sector'].isin(sector_options)]['ticker'].unique()
    elif (sector_options == [] or sector_options == None) and len(industry_options) >=1:
        new_tickers = growth_df[growth_df['industry'].isin(industry_options)]['ticker'].unique()
    else:
        new_tickers = growth_df[(growth_df['sector'].isin(sector_options)) & (growth_df['industry'].isin(industry_options))]['ticker'].unique()
    new_tickers.sort()
    return new_tickers
...
def update_icicle_chart(sector_options, industry_options, ticker_options):
    ...
    fig = px.icicle(df, path=['all', 'sector', 'industry', 'ticker'], values='close_pct_1yr')
...
def update_analyst_scatter_chart(sector_options, industry_options, ticker_options):
    ...
    fig = px.scatter(
        df, x="numberOfAnalystOpinions", y="targetMedianPrice", 
        color="sector", 
