...
def dashboard_tables(main_df, names, std_df=std_df):
    ...
    for name in names:
        df.loc[name]['1Wk Delta'] = df.loc[name]['Level'] - main_df[name][:-5].tail(1).item()
...
# Rates Tables
treas_rates_table = dashboard_tables(macro_df, ['2yTreas', '5yTreas', '10yTreas', '30yTreas', '30yr Mortgage'])
...
# FX Table
currency_table = dashboard_tables(macro_df, ['EURUSD', 'USDGBP', 'CHFUSD', 'USDJPY', 'CADUSD', 'MXNUSD', 'USDYUAN'])
...
layout = html.Div(children=[
    ...
    html.P(),
                        ...
                        html.Hr(),
                        html.Center(html.Div('''The Volatility-Adjusted Macro Dasboard displays macroeconomic variables relevant to market 
                        ...
                        dbc.Row(children=[
                            dbc.Col(children=[
                                html.Center(html.Div('Rates'))
                            ], width=3),
                            dbc.Col(children=[
                                html.Center(html.Div('Equities'))
                            ], width=3),
                            dbc.Col(children=[
                                html.Center(html.Div('Credits'))
                            ], width=3),
                            dbc.Col(children=[
                        ...
                        dbc.Row(children=[
                            dbc.Col(children=[
                                ...
                                html.P(),
                                dbc.Table.from_dataframe(treas_rates_table, striped=True, bordered=True, hover=True, responsive=True),
                                html.P(),
                                html.Center(html.P('US Treasury Curve')),
                                dbc.Table.from_dataframe(curve_rates_table, striped=True, bordered=True, hover=True, responsive=True),
                                html.P(),
                                html.Center(html.P('Inflation & Real Rates')),
                                dbc.Table.from_dataframe(ilbe_rates_table, striped=True, bordered=True, hover=True, responsive=True),
                            ], width=3),
                            dbc.Col(children=[
                            ...
                            dbc.Col(children=[
                                html.Center(html.Div('US')),
                                html.P(),
                                dbc.Table.from_dataframe(baml_rates_table, striped=True, bordered=True, hover=True, responsive=True),
                                html.P(),
                                dbc.Table.from_dataframe(corp_rates_table, striped=True, bordered=True, hover=True, responsive=True),
                            ], width=3),
                            dbc.Col(children=[
                                html.Center(html.Div('FX')),
                                html.P(),
                                html.P(),
                                dbc.Table.from_dataframe(currency_table, striped=True, bordered=True, hover=True, responsive=True),
                                html.P(),
                                html.Center(html.P('Commodities')),
                                dbc.Table.from_dataframe(commodities_table, striped=True, bordered=True, hover=True, responsive=True),
                            ], width=3)
                        ])
