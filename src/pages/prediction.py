...
def generate_line_graph(df, x, y, title):
    ...
    fig.update_layout(title=dict(text=title, x=.5, xanchor='center'), template='plotly_dark', 
                        showlegend=True, margin=dict(b=20, t=50, l=40, r=20), paper_bgcolor='rgba(0,0,0,0)',plot_bgcolor='rgba(0,0,0,0)')

    fig.update_traces(connectgaps=True)

    return fig

graph_df = pd.read_csv('src/predictions/data/model_data_1d.csv')
dl_df = pd.read_csv('src/predictions/preds/keras_preds.csv')
xgb_df = pd.read_csv('src/predictions/preds/xgboost_preds.csv')
cs_df = pd.read_csv('src/predictions/preds/cs_preds.csv')
...
dl_graph.add_trace(go.Scatter(x=dl_level_df['Date'], y=dl_level_df['Mean'], name='Mean'))
...
xgb_graph.add_trace(go.Scatter(x=xgb_level_df['Date'], y=xgb_level_df['Mean'], name='Mean'))
...
cs_graph.add_trace(go.Scatter(x=cs_level_df['Date'], y=cs_level_df['Mean'], name='Mean'))
                ...
                dcc.Tabs(children=[
                    dcc.Tab(id='nn-ml-tab', label='Deep Learning Model', style=TAB_STYLE, selected_style=TAB_SELECTED_STYLE, children=[
                        ...
                        html.Div('''Our deep learing model utilizes a multi-layer perceptron (implemented via Keras) that takes as input 
                                    ...
                                    given current data.'''),
                        html.P(),
                        html.Center(html.H4('S&P 500 Forward Trajectory - Deep Learning Model')),
                        html.P(),
                    ...
                    dcc.Tab(id='xgb-ml-tab', label='Boosted Tree Model', style=TAB_STYLE, selected_style=TAB_SELECTED_STYLE, children=[
                        ...
                        html.Div('''Our machine learning model utilizes a gradient-boosted tree model (implemented via XGBoost) that takes 
                                    ...
                                    given current data.'''),
                        html.P(),
                        html.Center(html.H4('S&P 500 Forward Trajectory - Gradient-Boosted Tree Model')),
                        html.P(),
                    ...
                    dcc.Tab(id='cs-ml-tab', label='Cosine Similarity', style=TAB_STYLE, selected_style=TAB_SELECTED_STYLE, children=[
                        ...
                        html.P(),
                        html.Center(html.H4('S&P 500 Forward Trajectory - Cosine Similarity Model')),
                        html.P(),
                        dcc.Graph(figure=cs_graph)

                    ]),
                    dcc.Tab(id='mass-tab', label='MASS Pattern Matching', style=TAB_STYLE, selected_style=TAB_SELECTED_STYLE, children=[

                        html.P(),
                        html.Div('''In an attempt to identify analogous historical periods to today's market, we employ an algorithm below
...
def mass_graph(cal_date):
    ...
    add = 200
    spx_past_z_norm = pd.DataFrame(stumpy.core.z_norm(spx_past["5DMA"].values[idx:idx+len(spx_sel)+add]))
    ...
    fig.update_layout(template='plotly_dark', showlegend=False, margin=dict(b=20, t=50, l=40, r=20), paper_bgcolor='rgba(0,0,0,0)',plot_bgcolor='rgba(0,0,0,0)')
