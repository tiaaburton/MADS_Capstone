...
def create_combined_dataframe(ticker, start_date, end_date):
    ...
    yahoo_df.drop(['targetLowPrice', 'targetMeanPrice',	'targetMedianPrice', 'targetHighPrice', 'numberOfAnalystOpinions',	'bookValue', 'targetMedianGrowth'], axis=1, inplace=True)
    # print(yahoo_df)
    
    fill_values = {"close_pct_1d": 0, "close_pct_30d": 0, "close_pct_60d": 0, "close_pct_120d": 0, "close_pct_1yr": 0}
    ...
    if sec_df is not None:
        ...
        combined_df = combined_df.merge(new_df, how='left', left_index=True, right_index=True)
...
def train_test_split(df):
    ...
    X_train, X_test, y_train, y_test, dates_train, dates_test = None, None, None, None, None, None
...
def create_combined_results_df(df, dates_test, y_hat, dates_pred, y_pred, ticker, sector, industry, train_score, test_score):
...
def create_random_forest_model(df):
    ...
    # Fit the model
    regr = RandomForestRegressor(n_estimators=200, max_depth=5, random_state=42, warm_start=False)
...
def create_gradient_boosted_tree_model(df):
    ...
    # Fit the model
    regr = GradientBoostingRegressor(n_estimators=750, max_depth=5, random_state=42, min_samples_split=5, learning_rate=0.02, loss="ls")
    regr.fit(X_train, y_train)
    train_score = regr.score(X_train, y_train)
...
def create_neural_network_model(df):
    ...
    # Fit the model
    regr = MLPRegressor(random_state=42, max_iter=1000, early_stopping=True, learning_rate='adaptive')
    regr.fit(X_train, y_train)
    train_score = regr.score(X_train, y_train)
