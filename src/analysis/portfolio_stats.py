...
def get_portfolio_data(
    ...
    portfolio['Sector'] = portfolio['Symbol'].apply(
        lambda ticker: get_json(f"https://finance.yahoo.com/quote/{ticker}")['summaryProfile']['sector']
    )
    portfolio['Price'] = portfolio['Symbol'].apply(
        lambda ticker: get_json(f"https://finance.yahoo.com/quote/{ticker}")['price']['regularMarketPrice']
    )
    denominator = (portfolio['Price'].values * portfolio['Share'].values).sum()
    portfolio['Weight'] = np.round_((portfolio['Price'].values * portfolio['Share'].values) / denominator, 2)
    portfolio['P/L'] = np.round_((portfolio['Price'].values - portfolio['Cost'].values) / portfolio['Cost'].values, 2)
    return portfolio
        ...
        fig.add_trace(
            go.Table(
                ...
                cells=dict(
                    ...
                    fill_color=[[rowOddColor, rowEvenColor, rowOddColor, rowEvenColor, rowOddColor] * len(self.data)],
                    align=['center'],
