...
class kdjChart:
    ...
    def create_chart(self):
        ...
        if (signal2 > signal1) and (signal1 > self.data['kdj.d'].iat[-1]) and (kdj > 80).all():
            signal2 = 'might be a good time to buy'
        elif (signal2 < signal1) and (signal1 < self.data['kdj.d'].iat[-1]) and (kdj < 20).all():
...
class movingAvgChart:
    ...
    def createWMA(self,
        ...
        fig = go.Figure()
        fig.add_trace(
            go.Scatter(x=stock_data.Date, y=stock_data[window], name=window)
        )
        fig.add_trace(
            go.Scatter(x=stock_data.Date, y=stock_data.Close, name='Close')
        )
